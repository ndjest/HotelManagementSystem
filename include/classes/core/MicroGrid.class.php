 ...
 *      - changed mysq l_error() with database_error();, mysq l_insert_id() with database_insert_id()
...
class MicroGrid {
	...
	public function GetFieldInfo($field = '')
	{
		if(isset($_POST[$field])){
			return $_POST[$field];
		}else if(isset($this->result[0][0][$field])){
		...
		$sql .= 'NULL';
			foreach($this->params as $key => $val){
				if(array_key_exists($key, $this->arrAddModeFields)){
					...
					}else if(in_array($this->arrAddModeFields[$key]['type'], array('date', 'datetime')) && $val === null){
						$sql .= ', NULL';
					}else{
						$sql .= ', \''.encode_text($val).'\'';
					}										
					...
					foreach($this->arrAddModeFields as $v_key => $v_val){
						if(array_key_exists($key, $v_val)){
							...
							}else if(in_array($v_val[$key]['type'], array('date', 'datetime')) && $val === null){
								$sql .= ', NULL';
							}else{
								$sql .= ', \''.encode_text($val).'\'';
							}					
		...
		$sql = 'UPDATE `'.$this->tableName.'` SET ';
			...
			foreach($this->params as $key => $val){
				if(array_key_exists($key, $this->arrEditModeFields)){
					...
					}else if(in_array($this->arrEditModeFields[$key]['type'], array('date', 'datetime')) && $val === null){
						$sql .= '`'.$key.'` = NULL';
					}else{
	...
	{        
				...
				foreach($this->arrFilteringFields as $key => $val){
					...
					if(!$custom_handler && self::GetParameter('filter_by_'.$val['table'].$val['field'], false) !== ''){
						...
						if($val['sign'] == '='){
							...
							$sign_start = '%';
							$sign_end = '%';
						}else if($val['sign'] == 'not_exists'){
                            $sign = 'IS NULL';
						}else if($val['sign'] == 'exists'){
                            $sign = 'IS NOT NULL';
						}
		...
		if($this->drawResponsiveWrapper) echo '<div class="table-responsive view-mode">';
	...
	{		
		...
		if($this->drawResponsiveWrapper) echo '<div class="table-responsive add-mode">';
	...
	{		
		...
		if($this->drawResponsiveWrapper) echo '<div class="table-responsive edit-mode">';
		echo '<form name="frmMicroGrid_'.$this->tableName.'" id="frmMicroGrid_'.$this->tableName.'" action="'.$this->formActionURL.'" method="post" enctype="multipart/form-data">'.$nl;
		draw_hidden_field('mg_prefix', $this->uPrefix); echo $nl;
					...
					if($data_type == 'serialized'){
						...
						if(is_array($field_values)){
							foreach($field_values as $key => $val){
								if(isset($source[$val])){
									$value = $source[$val];
									$output .= $pre_html.$this->FormatFieldValue($value, $format, $format_parameter).$post_html.'<br>';
								}
							}
						}
					}else{
						...
						if($mode == 'add' || $mode == 'edit'){
                            if($view_type == 'checkboxes'){
                                ...
                                foreach($source as $key => $val){
                                    ...
                                    $output .= '<div style="float:'.Application::Get('defined_left').';width:220px;"><div class="checkbox"><input type="checkbox" name="'.$field_name.'[]" id="'.$field_name.$checkboxes_count.'" value="'.$key.'" '.$checked.'/> <label for="'.$field_name.$checkboxes_count.'">'.$val.'</label></div></div>';
                                    $checkboxes_count++;
                                }
	...
	private function ValidateField($key, $val)
	{
		...
		if(isset($this->params[$key]) && $this->params[$key] != ''){		
			if($validation_type == 'email' && !$this->IsEmail($this->params[$key])){
				...
				return false;
			}else if($validation_type == 'url' && !$this->IsUrl($this->params[$key])){
				$this->error = str_replace('_FIELD_', '<b>'.$val['title'].'</b>', _FIELD_MUST_BE_URL);
				return false;				
			}else if($validation_type == 'date' && !empty($this->params[$key]) && !$this->IsDate($this->params[$key])){
							...
							if(isset($_FILES[$v_key]['name'])){
								...
								if($v_val['type'] == 'image'){
									if(!empty($_FILES[$v_key]['tmp_name']) && !getimagesize($_FILES[$v_key]['tmp_name'])){
										...
										return false;
									}else if(!empty($_FILES[$v_key]['name']) && $image_file_type != 'jpg' && $image_file_type != 'jpeg' && $image_file_type != 'png' && $image_file_type != 'gif' && $image_file_type != 'bmp'){
										$this->error = _INVALID_IMAGE_FILE_TYPE;
										return false;
									}
					...
					if(isset($_FILES[$key]['name'])){
						...
						if($val['type'] == 'image'){
							if(!empty($_FILES[$key]['tmp_name']) && !getimagesize($_FILES[$key]['tmp_name'])){
								$this->error = _INVALID_IMAGE_FILE_TYPE;
								return false;
							}else if(!empty($_FILES[$key]['name']) && $image_file_type != 'jpg' && $image_file_type != 'jpeg' && $image_file_type != 'png' && $image_file_type != 'gif' && $image_file_type != 'bmp'){
								$this->error = _INVALID_IMAGE_FILE_TYPE;
								return false;
							}
