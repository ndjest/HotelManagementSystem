if(hsJsHost != ''){
	...
	var hsHotelIDs = (typeof hsHotelIDs === 'undefined') ? '' : hsHotelIDs;
	
	if(hsJsKey != '' && hsJsHost != ''){
		
		var encoded_host = encode64(hsJsHost);
		var encoded_key = encode64(hsJsKey);
		var encoded_ids = hsHotelIDs != '' ? encode64(hsHotelIDs.replace(",","-").replace(" ","")) : '';
		
		var filePath = 'widgets/hotels/ipanel-side/index.php?host='+encoded_host+'&key='+encoded_key+'&hids='+encoded_ids;
		
		// setup the iframe target
		var iframe='<iframe id="frame" name="widget" src="#" width="230px" height="320px" marginheight="0" marginwidth="0" frameborder="no" scrolling="no"></iframe>';
