...
class Rating {
  ...
  public $rtgitems = 'ratings_items';        // Table /or file_name to store items that are ranted
  public $rtgusers = 'ratings_users';        // Table /or filename that stores the users who rated in current day
  ...
  public function __construct() {
    ...
    $this->rtgitems = DB_PREFIX.'ratings_items';        // Table /or file_name to store items that are ranted
    $this->rtgusers = DB_PREFIX.'ratings_users';        // Table /or filename that stores the users who rated in current day
    ...
    if($this->svrating != 'mysql') {
      $this->rtgitems = '../data/'.$this->rtgitems.'.txt';
      $this->rtgusers = '../data/'.$this->rtgusers.'.txt';
    }
  }
  ...
  protected function setConn() {
    try {
      ...
      self::$conn = new PDO("mysql:host=".DATABASE_HOST."; dbname=".DATABASE_NAME, DATABASE_USERNAME, DATABASE_PASSWORD);
  ...
  public function getRating($items, $rate = '') {
    ...
    if(!empty($rate)) {
      ...
      if($this->rater === '') return 'alert(RatingsVoc._MSG["vote_not_registered"])';
      else {
  ...
  protected function getRtgDb($items, $rtgstdy, $setrated) {
	...
	$in_values = !empty($items) ? implode(',', array_map('addSlhs', $items)) : '-1';
    $resql = $this->sqlExecute("SELECT * FROM `$this->rtgitems` WHERE `item` IN(".$in_values.")");
